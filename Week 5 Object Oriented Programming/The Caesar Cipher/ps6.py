...
def load_words(file_name):
    ...
    # line: string
    line = in_file.readline()  # read an entire line
    # word_list: list of strings
    word_list = line.split()  # return a list of strings after breaking the given string by any white space 
...
def is_word(word_list, word):
    ...
    '''
    word = word.lower()
    word = word.strip(" !@#$%^&*()-_+={}[]|\:;'<>?,./\"") # remove all leading and trailing spaces on the word
...
def get_story_string():
    ...
    """
    f = open("story.txt", "r")
    story = str(f.read())  # read entire file
    f.close()
    return story
...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(len(lowercase_letters)):
            shifted_letters[lowercase_letters[i]] = lowercase_letters[(i + shift) % 26]
            
        for i in range(len(uppercase_letters)):
            shifted_letters[uppercase_letters[i]] = uppercase_letters[(i + shift) % 26]
...
##    
    def apply_shift(self, shift):
        ...
        Returns: the message text (string) in which every character is shifted
             down the alphabet by the input shift
        '''
        shifted_dict = Message.build_shift_dict(self, shift)
        shifted_text = ''
...
class PlaintextMessage(Message):
    ...
    def get_message_text_encrypted(self):
        ...
        '''
        return self.apply_shift(self.get_shift()) # since self is an instance of the Message class hence we can access it methods 
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            ...
            for word in words:
                if is_word(self.valid_words, word):    # OR: if word in valid_words:  
                    temp_count += 1
            if temp_count > valid_words_count:
                valid_words_count = temp_count
                best_shift = i
